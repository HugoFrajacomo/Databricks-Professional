...
# MAGIC %sql
# MAGIC -- NOTA: Esta célula contém a consulta SQL para sua referência e não funcionará se for executada diretamente. 
# MAGIC -- A consulta é utilizada abaixo na função type2_upsert() como parte da chamada foreachBatch.
# MAGIC
# MAGIC MERGE INTO books_silver
# MAGIC USING (
# MAGIC     SELECT updates.book_id as merge_key, updates.* --id de referencia
# MAGIC     FROM updates
# MAGIC
# MAGIC     UNION ALL
# MAGIC     
...
# MAGIC WHEN MATCHED AND books_silver.current = true AND books_silver.price <> staged_updates.price THEN --Caso o arquivo atual tenha preço diferente do preço anterior, ele será atualizado
# MAGIC   UPDATE SET current = false, end_date = staged_updates.updated -- Arquivo anterior será atualizado com a data de fim e setado como false
# MAGIC WHEN NOT MATCHED THEN -- Caso contrário será inserido um novo registro
...
# MAGIC %md
# MAGIC ## Filtrando apenas os dados atuais

# COMMAND ----------

# MAGIC %sql
# MAGIC CREATE OR REPLACE TABLE current_books
# MAGIC AS SELECT book_id, title, author, price --Utilizamos apenas os campos específicos da tabela
